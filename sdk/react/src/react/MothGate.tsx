 ...
 * the failure.
 */
const gateVerdicts = new WeakMap<MothClient, Map<string, Promise<boolean>>>()

function resolveGateBlocks(
  client: MothClient,
  entitlement: string,
): Promise<boolean> {
 ...
 */
export function MothGate(props: MothGateProps) {
  const { client } = useMothContext()
  const { active } = useMothEntitlement(props.entitlement)
  const [resolution, setResolution] = useState<'resolving' | 'blocks' | 'open'>(
    'resolving',
  )

  useEffect(() => {
    if (active) return // nothing to resolve while the entitlement is held
    let cancelled = false
    let timer: ReturnType<typeof setTimeout> | undefined
    let delay = retryBaseDelayMs
    const attempt = () => {
      resolveGateBlocks(client, props.entitlement).then(
        (blocks) => {
          if (!cancelled) setResolution(blocks ? 'blocks' : 'open')
        },
        () => {
          if (cancelled) return
  ...
  if (resolution === 'open') return <>{props.children}</>
