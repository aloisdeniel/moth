// Minimal synchronous SHA-256, used only to derive cache-namespace keys from
// the publishable key (sha256(pk) hex, first 16 chars). SubtleCrypto is
// async-only, and the caches want a synchronous key at construction time, so
// a tiny embedded implementation is the simpler dependency-free answer.
