...
import { BlobCache, cacheNamespace, defaultCacheStorage, paywallCacheKey, } from './cache.js';
import { configCacheTtlMs } from './config.js';
import { emptyPaywall, paywallFromProto } from './offering.js';
/**
 ...
 */
export async function loadPaywall(client, options = {}) {
    const storage = options.storage ?? defaultCacheStorage();
    const now = options.now ?? (() => Date.now());
    const cache = new BlobCache(storage, paywallCacheKey(cacheNamespace(client.config.publishableKey)));
    ...
    try {
        ...
        if (cached !== null)
            return cached; // offline: stale beats nothing
        throw err;
    }
    const stamp = now();
    if (fetched === null) {
