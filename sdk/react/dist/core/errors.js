 ...
 */
export function mapConnectError(error) {
 ...
 */
export function mothReasonOf(error) {
    for (const detail of error.details) {
        if ('type' in detail && typeof detail.type === 'string') {
            ...
            if (info.domain === mothErrorDomain && info.reason)
                return info.reason;
        }
        else if ('desc' in detail) {
            // Outgoing detail (in-process transports): schema + init shape.
            if (detail.desc.typeName !== 'google.rpc.ErrorInfo')
                continue;
            const value = detail.value;
            if (value.domain === mothErrorDomain && value.reason)
                return value.reason;
        }
    }
    return undefined;
...
export function decodeErrorInfo(bytes) {
    const out = {};
    let i = 0;
    ...
    while (i < bytes.length) {
        ...
        if (wireType === 2) {
            ...
            i += len;
            if (field === 1)
                out.reason = decoder.decode(value);
            else if (field === 2)
                out.domain = decoder.decode(value);
        }
        else if (wireType === 0) {
            for (;;) {
                const b = bytes[i];
...
export function encodeErrorInfo(reason, domain) {
